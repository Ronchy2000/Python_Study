...
def myMLPRegressor(x,y):
    ...
    real_data = np.concatenate((real_data, testY), axis=1)  # 按列拼接
    sc=StandardScaler()
...
if __name__ == "__main__":
    ...
    for i in range(df_data5.shape[1]):
        ...
        for j in data_target.T:  # 对 列 进行迭代
            tmp_mae, tmp_rmse, len_less10,predict = myMLPRegressor(data_feature, j.reshape(-1, 1))
    ...
    dff2.to_csv("b19_15nm_v5_prediction_MLP.csv", sep=',', index=False)
    dff3 = pd.DataFrame(real_data)
    dff3.to_csv("b19_15nm_v5_real_MLP.csv", sep=',', index=False)
    print("pridiction siteration:", len(MAE))  # 13*14 次
    result_mae = sum(MAE) / len(MAE)
    print("MAE", result_mae)
