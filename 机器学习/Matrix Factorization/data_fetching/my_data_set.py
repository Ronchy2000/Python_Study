...
class DataSet:
    ...
    ## Dataset constants
    DATASETS = ['movielens', 'jester', 'toy','Corner']  ## All datasets
    DATASETS_WITH_SIZE = ['movielens','Corner']
    ...
    def __init__(self, dataset='Corner', size='14', u=100, i=1000, u_unique=10, i_unique=50, density=0.2, noise=0.3,
                 score_low=1, score_high=5, strong_gen=False, users_size=1.):
        """
        @Parameters:
        ------------
        ...
        self.nb_items = len(np.unique(self.df[DataSet.ITEM_ID]))  #DataSet.ITEM_ID - col
...
# ========================================================================
    ...
    def split_train_test(self, strong_generalization=False, train_size=0.8, users_size=1.):
        ...
        if strong_generalization:
            user_id_train_set = np.random.choice(unique_user_id, size=int(train_size * len(unique_user_id)),
                                                 replace=False)
            ...
            idx_heldout_test_set = [np.random.choice(test_set_df[test_set_df[DataSet.USER_ID] == idx].index) for idx in
                                    user_id_test_set]
            ...
            # Weak generalization
            idx_heldout_test_set = [np.random.choice(self.df[self.df[DataSet.USER_ID] == idx].index) for idx in
                                    unique_user_id]
            idx_unique_user_selected = np.concatenate(
                [self.df[self.df[DataSet.USER_ID] == idx].index for idx in unique_user_id])
    ...
    def __set_df(self):
        ...
        if self.dataset in DataSet.DATASETS_TO_BE_FETCHED:
            ...
            df_complete, df = self.__get_df_toy(self.__u, self.__i, self.__u_unique, self.__i_unique,
                                                self.density, self.__noise, self.score_low, self.score_high,
                                                out="dataframe")
    ...
    def __get_df_toy(self, u, i, u_unique, i_unique, density, noise, score_low, score_high, out):
        ...
        ratings = np.round(
            np.clip(
                np.fromfunction(
                    np.vectorize(lambda i, j: rating_unique_matrix[X[i]][Y[j]] + (
                        np.random.normal(0, noise) if noise > 0 else 0)),
                    (u, i),
                    dtype=int
                ),
                score_low,
        ...
        df_nan[DataSet.RATING] = df_nan.apply(lambda row: ratings_nan[row[DataSet.USER_ID]][row[DataSet.ITEM_ID]],
                                              axis=1)
