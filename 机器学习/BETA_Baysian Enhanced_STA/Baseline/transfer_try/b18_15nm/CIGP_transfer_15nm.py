# -*- coding: utf-8 -*-
# @Time    : 2022/9/22 18:44
# @Author  : Ronchy_LU
# @Email   : rongqi1949@gmail.com
# @File    : CIGP_transfer_15nm.py
# @Software: PyCharm
...
if __name__ == "__main__":
    ...
    for i in range(5):
        ...
        # total_data = np.concatenate((df_data1[:500,:], df_data2[:500,:], df_data3[:500,:]), axis=0)
        total_data = np.concatenate((df_data1[:num_of_key_sample,:], df_data3[:num_of_key_sample,:], df_data2[:num_of_key_sample,:]), axis=0)
        ...
        list_result_less10 = []
        Covariance_pred = np.arange(0,num_of_key_sample).reshape(-1,1)
        mean_predict = np.arange(0,num_of_key_sample).reshape(-1,1)

        print("Covariance_pred.shape:",Covariance_pred.shape)

        data_feature = total_data[:,one_Corner].reshape(-1,1) #第 1 列
        ...
        for j in data_target.T:  #对 列 进行迭代
            ...
            xtr = data_feature[:int(total_data.shape[0]*(2/3))]
            ytr = j[:int(total_data.shape[0]*(2/3))].reshape(-1,1)
            xte = data_feature[int(total_data.shape[0]*(2/3)) : ]
            yte = j[int(total_data.shape[0]*(2/3)) :].reshape(-1,1)
        ...
        print("b18_v1_15nm BenchMark Done.")
        print("==========next starting...=============")
...
#plot
    x = [ i for i in range(len(result_MAE_plot)) ]
    plt.plot(x,result_MAE_plot)
    plt.plot(x,result_RMSE_plot)
    plt.show()
