...
def linear3(data_feature,data_target):
    ...
    global real_data
    real_data = np.concatenate((real_data, y_test), axis=1)  # 按列拼接
...
#**********************************************
    ...
    return mae, rmse, less10,y_predict
...
#     result_mae, result_rmse, result_less10,LESS10 = 0,0,0,0
    ...
    for i in range(df_data5.shape[1]):
        ...
        for j in data_target.T:  # 对 列 进行迭代
            tmp_mae, tmp_rmse, len_less10,predict = linear3(data_feature, j.reshape(-1, 1))
            mean_predict = np.concatenate((mean_predict, predict), axis=1)  # 按列拼接
            MAE.append(tmp_mae)
            RMSE.append(tmp_rmse)
            LESS10 += len_less10
    ...
    dff2.to_csv("b19_15nm_v5_prediction_Ridge.csv", sep=',', index=False)
    dff3 = pd.DataFrame(real_data)
    dff3.to_csv("b19_15nm_v5_real_Ridge.csv", sep=',', index=False)
    print("pridiction siteration:", len(MAE))  # 13*14 次
    result_mae = sum(MAE) / len(MAE)
    print("MAE", result_mae)
